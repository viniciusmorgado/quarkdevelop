#!/usr/bin/env bash
# Full MonoDevelop.Ide compile that reaches method bodies: temporarily replaces Gtk3PortPending.props by
# one that excludes only the ResX generator files (ported separately), then restores it.
