...
namespace Microsoft.VisualStudio.Text.Operations.Implementation
{
    ...
    internal class EditorOperations : IEditorOperations4
    {
        ...
        public bool Paste()
        {
            ...
            try
            {
                // MonoDevelop local patch: host clipboard instead of the WPF one (EditorClipboard.cs).
                text = EditorClipboard.Current.GetText(out dataHasLineCutCopyTag, out dataHasBoxCutCopyTag);
                if (text == null)
                {
                    return true;
                }
        ...
        public bool CanPaste
        {
            get
            {
                ...
                try
                {
                    return EditorClipboard.Current.ContainsText() && !_textView.TextSnapshot.TextBuffer.IsReadOnly(_editorPrimitives.Caret.CurrentPosition);
                }
                catch (System.Runtime.InteropServices.ExternalException)
                {
                    // TODO: Log error
        ...
        // MonoDevelop local patch: the MaxZoom/MinZoom extensions live in the WPF text UI assembly.
        public bool CanZoomIn => CanZoomTo && _factory.ZoomManager.ZoomLevel(_textView) < _textView.Options.GlobalOptions.GetOptionValue(DefaultTextViewOptions.MaxZoomLevelId);

        public bool CanZoomOut => CanZoomTo && _factory.ZoomManager.ZoomLevel(_textView) > _textView.Options.GlobalOptions.GetOptionValue(DefaultTextViewOptions.MinZoomLevelId);
        ...
        private static bool CopyToClipboard(string textData, string rtfData, bool lineCutCopyTag, bool boxCutCopyTag)
        {
            ...
            {
                // MonoDevelop local patch: host clipboard instead of the WPF one (EditorClipboard.cs).
                EditorClipboard.Current.SetText(textData, rtfData, lineCutCopyTag, boxCutCopyTag);
