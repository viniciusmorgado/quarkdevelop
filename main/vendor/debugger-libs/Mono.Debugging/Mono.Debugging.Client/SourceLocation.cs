...
namespace Mono.Debugging.Client
{
	...
	public class SourceLocation
	{
		...
		// MonoDevelop: HashAlgorithm.Create (string) is obsolete on .NET (SYSLIB0045); same lookup through CryptoConfig.
		...
		static bool CheckHash (Stream stream, string algorithm, byte[] checksum)
		{
			using (var hash = CreateHash (algorithm)) {
				...
				using (var dos = CreateHash (algorithm)) {
					using (var unix = CreateHash (algorithm)) {
						stream.Position = 0;
