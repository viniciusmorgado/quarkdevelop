...
type InteractiveSession(pathToExe) =
    ...
    let fsiProcess =
        ...
        // .NET: the dotnet host runs the service assembly, as for the MSBuild builder
        let processName = MonoDevelop.Core.Assemblies.DotNetCoreSdkInfo.GetDotNetHostPath()

        let arguments = "exec " + pathToExe + processPid
