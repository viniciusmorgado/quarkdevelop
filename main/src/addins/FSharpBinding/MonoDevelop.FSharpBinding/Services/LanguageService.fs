...
type ParseAndCheckResults (infoOpt : FSharpCheckFileResults option, parseResults : FSharpParseFileResults option) =
    ...
    member x.GetUsesOfSymbolAtLocationInFile(fileName, line, col, lineStr) =
        asyncMaybe {
            LoggingService.logDebug "LanguageService: GetUsesOfSymbolAtLocationInFile: file:%s, line:%i, col:%i" (Path.GetFileName(fileName: string)) line col
...
type LanguageService(dirtyNotify, _extraProjectInfo) as x =
    ...
    let projectChecked filename =
        let computation =
            async {
                let displayname = Path.GetFileName (filename: string)
    ...
    member x.GetScriptCheckerOptions(fileName, projFilename, source) =
        let opts =
            ...
            try
                ...
                // .NET: scripts reference the assemblies of the running runtime, as dotnet fsi does; the .NET Framework
                // default finds no framework on Linux, and the check is aborted
                let opts, _errors =
                  Async.RunSynchronously (checker.GetProjectOptionsFromScript(fileName, SourceText.ofString source, fakeDateTimeRepresentingTimeLoaded projFilename,
                                                                              otherFlags = [| "--targetprofile:netcore" |], assumeDotNetFramework = false),
    ...
    member x.ParseAndCheckFileInProject(projectFilename, fileName, version:int, src:string, obsoleteCheck) =
        let fileName = if Path.GetExtension (fileName: string) = ".sketchfs" then Path.ChangeExtension (fileName, ".fsx") else fileName
