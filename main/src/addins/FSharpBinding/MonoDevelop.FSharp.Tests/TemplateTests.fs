...
type ``Template tests``() =
    ...
    // Quarantine (T134): portable class libraries are retired, and the F# templates come from dotnet new (T152)
    [<Category("Quarantine")>]
    ...
    [<Ignore("Ignored in the NUnit 2 suite, no reason given");AsyncStateMachine(typeof<Task>)>]member x.``Xamarin Mac FSharp CocoaApp-XIB``()= test "Xamarin.Mac.FSharp.CocoaApp-XIB"
