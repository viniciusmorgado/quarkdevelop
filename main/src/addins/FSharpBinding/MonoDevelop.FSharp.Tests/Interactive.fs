...
module Interactive =
    ...
    let createSession() =
        async {
            ...
            let pathToExe = "\"" + testDllFolder/"MonoDevelop.FSharpInteractive.Service.dll\""
    ...
    [<Test;AsyncStateMachine(typeof<Task>)>]
    // Quarantine (T134): the bug43307 fixture is a .NET Framework 4.5.1 project; F# Interactive runs on .NET
    [<Category("Quarantine")>]
