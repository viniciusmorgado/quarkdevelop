...
- **T142.** Core tests of Mono features that Linux/.NET does not have: `ProjectTests.RefreshReferences` expects a missing local `gtk-sharp.dll` to fall back to the Mono GAC package (no GAC, ADR 0007); `MakefileTests.MakefileSynchronization` needs `MonoDevelop.Autotools` (removed, ADR 0017) → rewrite RefreshReferences on an assembly of the framework packages (fixture `main/tests/test-projects/reference-refresh`); remove MakefileTests and `console-project-with-makefile`, whose add-in is removed and list them in BREAKING-CHANGES: `./scripts/pm bash -lc 'dotnet build main/tests/MonoDevelop.Core.Tests/MonoDevelop.Core.Tests.csproj -v q && dotnet test main/tests/MonoDevelop.Core.Tests/MonoDevelop.Core.Tests.csproj --no-build --filter "FullyQualifiedName~ProjectTests.RefreshReferences"'`
