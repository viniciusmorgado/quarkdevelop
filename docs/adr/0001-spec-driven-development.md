...
| 1.4.0 (MINOR) | Principle V: the global coverage target of 40% is dropped. Core stays ≥ 60%; further tests target critical methods and end-to-end runs of the IDE; the ratchet stays; existing tests are preserved. | Maintainer decision (2026-09-25): a smoothly running IDE is proven by functional tests first, not by a percentage. The total was 29.3% at M9; reaching 40% meant mostly GUI-level tests of low value per line. SC-003 amended accordingly. |
